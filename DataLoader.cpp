...
void MeteoDataLoader::CreateFronts (Direct3DProcessor* proc)
{
	...
	ShaderIndex_t vertS = proc->LoadShader ("shaders.hlsl",
											"VShader",
											SHADER_VERTEX);
	ShaderIndex_t pixS =  proc->LoadShader ("shaders.hlsl",
											"PShader",
											SHADER_PIXEL);
	/*ShaderIndex_t geoS = proc->LoadShader ("shaders.fx",
										   "GShader",
										   SHADER_GEOMETRY);*/
	...
	if (!object_)
		_EXC_N (NULL_FRONT_OBJECT, "object null");
	for (int i = 0; i < 3; i++)
	{
		...
		for (int x = 0; x < DATA_WIDTH; x++)
		{
			...
			{
				currentVertex = {};
				currentVertex.SetPos (-REGION_X / 2.0f + REGION_X / (1.0f * (DATA_WIDTH - 1))* x,
									  0.01f + i * 0.005f/*-REGION_Z / 2.0f + REGION_Z / (1.0f * (SLICES - 1))* i*/,
									  -REGION_Y / 2.0f + REGION_Y / (1.0f * (DATA_HEIGHT - 1))* y);


				//printf ("%f %f %f\n", \
						-REGION_X / 2.0f + REGION_X / (1.0f * DATA_WIDTH)* x, \
						-REGION_Y / 2.0f + REGION_Y / (1.0f * DATA_HEIGHT)* y, \
						-REGION_Z / 2.0f + REGION_Z / (1.0f * SLICES)* i);
				float color = data_.front(x, y, i, 0) / 11.0f;
				if (color > 1.2f) color = 0.0f;
				//if (color < 0.f) color = 0.0f;
				currentVertex.SetColor (color, color * (1.0f - i * 1.0f /SLICES), color/2.0f, 1.0f);
	...
	proc->AttachShaderToObject (object_, vertS);
	proc->AttachShaderToObject (object_, pixS);
	//proc->AttachShaderToObject (frontsObject_[i], geoS);
	proc->SetLayout (object_, n);
	proc->RegisterObject (object_);

	END_EXCEPTION_HANDLING (CREATE_OBJECTS)
}

void MeteoDataLoader::CreateCoords (Direct3DProcessor* proc)
{
	BEGIN_EXCEPTION_HANDLING
	...
	for (int x = 0; x < DATA_WIDTH; x++)
	{
		for (int y = 0; y < DATA_HEIGHT; y++)
		{
			...
			//if (color > 1.2f) color = 0.0f;
			//if (color < 0.f) color = 0.0f;
			currentVertex.SetColor (1.0f, 1.0f, 1.0f, 1.0f);
			vertices.push_back (currentVertex);
		}
	}
		//printf ("%d\n", vertices.size ());
...
void MeteoDataLoader::CreateGround (Direct3DProcessor* proc)
{
	BEGIN_EXCEPTION_HANDLING
	...
	float hMAX = data_.ground (0, 0);
	for (int x = 0; x < DATA_WIDTH; x++)
	{
		for (int y = 0; y < DATA_HEIGHT; y++)
		{
			if (data_.ground (x, y) < hMIN) hMIN = data_.ground (x, y);
			if (data_.ground (x, y) > hMAX) hMAX = data_.ground (x, y);
		}
	}
	...
	for (int x = 0; x < DATA_WIDTH; x++)
	{
		...
		{
			currentVertex = {};
			currentVertex.SetPos (-REGION_X / 2.0f + REGION_X / (1.0f * DATA_WIDTH)* x,
								  -(REGION_Z / 6.0f) + REGION_Z / hD * data_.ground (x, y),
								  -REGION_Y / 2.0f + REGION_Y / (1.0f * DATA_HEIGHT)* y);
...
void MeteoDataLoader::CreateAll (Direct3DProcessor* proc)
{
	BEGIN_EXCEPTION_HANDLING
	...
	//mapping[indices] = DATA_WIDTH * DATA_HEIGHT - 1;
	...
	//proc->AttachShaderToObject (object_, geoS_);
...
void MeteoDataLoader::BuildFront (Direct3DProcessor * proc)
{
	BEGIN_EXCEPTION_HANDLING
	...
	/*
	for (uint16_t slice = 0; slice < SLICES - 1; slice++)
	{
		for (uint16_t i = 0; i < interpolationN; i++)
		{
			currX = (data_.latitude (x, y) - xMIN);
			currY = (data_.longitude (x, y) - yMIN);
			for (uint8_t i = 0; i < SLICES; i++)
			{
				float color = data_.front (x, y, i, currentHour_) / 12.0f;
				if (color > 1.2f || color < 0.01f) continue;
				currentVertex.SetColor (0.0f, color, color, 1.0f);
...
void MeteoDataLoader::NextHour (Direct3DProcessor * proc)
{
	currentHour_++;
...
void MeteoDataLoader::LoadShadersAndLayout (Direct3DProcessor* proc)
{
